...
exports.inserUser = async ( req, res, next ) => {
        ...
        try{
                var query = `SELECT * FROM busers WHERE email = ?`;
                ...
                query = `INSERT INTO busers(name, email, role, password) VALUES (?,?,?,?)`;
                const results = await mysql.execute(query, [req.body.name, req.body.email, req.body.role, hash]);
...
exports.Login = async (req, res, next) => {
        ...
        try{
            const query = `SELECT * FROM busers WHERE email = ?`;
